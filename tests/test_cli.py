...
from contextpack_md.cli import app, clean_domain
...
def test_web_success():
    ...
    with patch("contextpack_md.cli.get_url_context") as mock_get:
...
def test_web_write(tmp_path):
    ...
    with patch("contextpack_md.cli.CONTEXTPACK_DIR", tmp_path):
        with patch("contextpack_md.cli.get_url_context") as mock_get:
            mock_get.return_value = content
            result = runner.invoke(app, ["web", url, "--write"])

            assert result.exit_code == 0
            assert "💾 Content written to:" in result.stdout
...
def test_web_clear(tmp_path):
    # Setup a dummy .contextpack-md folder
    ...
    with patch("contextpack_md.cli.CONTEXTPACK_DIR", tmp_path):
        ...
        assert result.exit_code == 0
        assert "🗑️  Clearing 1 item" in result.stdout
        assert not (tmp_path / "dummy.md").exists()
...
def test_pdf_success(tmp_path):
    ...
    with patch("contextpack_md.cli.CONTEXTPACK_DIR", tmp_path):
        with patch("contextpack_md.cli.download_pdf") as mock_download:
            with patch("contextpack_md.cli.convert_pdf_to_markdown") as mock_convert:
